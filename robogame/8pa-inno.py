...
async def main():
    ...
    # move to museum
    await move(113, Speed.Fast)
    # drop expert
    await motor.run_for_degrees(port.D, 180, 600)
    # solve purple guy
    await move(-6)
    runloop.run(motor.run_for_degrees(port.A, -360, 1000), motor.run_for_degrees(port.B, -360, 1000))
    await move(-5)
    # # go to light show
    await yaw(-110, Speed.Fast)
    await move(40, Speed.Fast)
    # # solve light show
    await motor.run_for_degrees(port.D, -2150, 800)

    # await move(-10)
    # await turn(45, Speed.Fast)
